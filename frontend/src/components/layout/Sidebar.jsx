    ...
    return (
        <aside className="filter-sidebar">
                ...
                <div className="checkbox-list">
                    <label>
                        <input
                            ...
                            onChange={() => handleCheckbox('conditions', 'New')}
                        /> New
                    </label>
                    <label>
                        <input
            ...
            <div className="filter-group">
                <h4>Price: ${filters.priceRange.min.toLocaleString()} - ${filters.priceRange.max.toLocaleString()}</h4>
                <div className="slider-container">
                    <input type="range" min="8000" max="700000" step="1000"
            ...
            <div className="filter-group">
                <h4>Year: {filters.yearRange.min} - {filters.yearRange.max}</h4>
                <div className="slider-container">
                    <input type="range" min="2000" max={nextYear} step="1"
                           className="thumb thumb-left"
            ...
            {/* ── KMS RANGE ── updated max to 200000 for real mileage values ── */}
            <div className="filter-group">
                <h4>KMs: {filters.kmsRange.min.toLocaleString()} - {filters.kmsRange.max.toLocaleString()}</h4>
                <div className="slider-container">
                    <input type="range" min="0" max="200000" step="1000"
                           className="thumb thumb-left"
                           value={filters.kmsRange.min}
                           onChange={(e) => setFilters({ ...filters, kmsRange: { ...filters.kmsRange, min: Math.min(Number(e.target.value), filters.kmsRange.max - 1000) } })}
                    />
                    <input type="range" min="0" max="200000" step="1000"
                           className="thumb thumb-right"
                           value={filters.kmsRange.max}
                           onChange={(e) => setFilters({ ...filters, kmsRange: { ...filters.kmsRange, max: Math.max(Number(e.target.value), filters.kmsRange.min + 1000) } })}
                    />
                    <div className="slider-track"></div>
                    <div className="slider-range-highlight" style={getSelectionStyles(filters.kmsRange.min, filters.kmsRange.max, 200000, 0)}></div>
                </div>
            </div>
