...
const emptyForm = {
  ...
  category: 'sedan', condition: 'used', color: '',
...
const CarListingEditor = () => {
  ...
  const handleEdit = (car) => {
    setFormData({
      ...
      price: car.price, mileage: car.mileage, category: car.category,
      condition: car.condition, color: car.color,
      transmission: car.transmission, fuel_type: car.fuel_type,
      engine: car.engine, description: car.description,
      image_url: car.image_url || '', video_url: car.video_url || '',
    });
    setEditingId(car.id);
    setShowForm(true);
    setError('');
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <select name="category" value={formData.category} onChange={handleChange}
                  ...
                  {['sedan', 'suv', 'truck', 'electric', 'coupe', 'van'].map(c => <option key={c} value={c}>{c}</option>)}
                </select>

                <select name="condition" value={formData.condition} onChange={handleChange}
                  style={{ padding: '10px', borderRadius: '8px', border: '1px solid rgba(128,128,128,0.3)', background: 'var(--bg-color)', color: 'var(--text-color)' }}>
                  {['new', 'used', 'featured'].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--accent-color)', textAlign: 'left' }}>
                {['ID', 'Make', 'Model', 'Year', 'Price', 'Category', 'Condition', 'Actions'].map(h => (
            ...
            <tbody>
              {cars.map((car) => (
                <tr key={car.id} style={{ borderBottom: '1px solid rgba(128,128,128,0.2)' }}>
                  ...
                  <td style={{ padding: '12px', textTransform: 'capitalize' }}>{car.condition}</td>
